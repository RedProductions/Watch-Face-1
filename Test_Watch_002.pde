...
//what time to update weather (5 bc api update's at 0)
final int weatherTime = 5;
...
void draw(){
  ...
  if(!wearAmbient()){
    ...
    if(steps < stepGoal){
      ...
      col = 145;
      stroke(col, 255, 255);
      arc(width/2, height/2, width/5 * 4, height/5 * 4, PI, PI + PI);
    }
    ...
    textSize(height/15);
    if(steps >= 0){
      text(steps, width/2, height/2 + height/8 + height/30);
    }else {
      text("-/-", width/2, height/2 + height/8 + height/30);
    }
  ...
  if(hour() == 1 && minute() == 0 && !resetStep){
    ...
    //reset avaukability to reset next day
    resetStep = true;
  }
...
void getWeather(){
  ...
  try{
    ...
    XML condition = xml.getChild("results/channel/item/yweather:forecast");
    
    // Get the attributes we want:
      //temperature (int)
      temperature = condition.getInt("high");
